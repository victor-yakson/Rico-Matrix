...
export function VotingExperience({
  ...
  const { data: totalUsdtCollected, refetch: refetchTotalUsdtCollected } = useReadContract({
    ...votingContract,
    functionName: 'totalUsdtCollected',
  });
  ...
  const needsApproval = totalCost > parsedUsdtAllowance;
  ...
  const globalUsdt = dashboardGlobal ? toBigInt(dashboardGlobal.totalUsdtCollected) : toBigInt(totalUsdtCollected);
    ...
    : registeredOnly && !isRegistered
      ...
      : activeAction === 'approve' || (isConfirming && needsApproval)
        ...
        : activeAction === 'vote' || (isConfirming && !needsApproval)
          ? t('actions.casting')
          : needsApproval
            ? t('actions.approveAmount', { amount: formatToken(totalCost) })
    ...
    try {
      if (needsApproval) {
        ...
        const approveHash = await writeContractAsync({
          ...
          args: [votingContract.address, totalCost],
        });
        if (publicClient) {
  ...
  return (
    <div className={`space-y-4 ${pageIsCompact ? 'max-h-[78vh] overflow-y-auto pr-1' : ''}`}>
      <section className="theme-panel relative overflow-hidden border border-yellow-400/20 bg-[radial-gradient(circle_at_top_right,rgba(245,166,35,0.14),transparent_24%),radial-gradient(circle_at_bottom_left,rgba(0,212,255,0.08),transparent_26%),linear-gradient(180deg,rgba(10,12,18,0.98),rgba(4,6,10,0.98))] p-4 md:p-5">
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
          <div>
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div className="max-w-3xl">
                ...
                <h2 className="text-2xl font-semibold leading-tight text-slate-50 md:text-[2rem]">
                  {t('hero.title')}
                </h2>
          ...
          <div className="grid gap-3">
                ...
                {countdown ? (
                  <>
                    <div className="rounded-2xl border border-white/10 bg-black/25 px-3 py-3 text-center">
                      ...
                      <p className="mt-1 text-xl font-semibold text-slate-50">{countdown.days}</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-black/25 px-3 py-3 text-center">
                      <p className="text-[10px] uppercase tracking-[0.18em] text-slate-400">{t('countdown.hours')}</p>
                      <p className="mt-1 text-xl font-semibold text-slate-50">{countdown.hours}</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-black/25 px-3 py-3 text-center">
                      <p className="text-[10px] uppercase tracking-[0.18em] text-slate-400">{t('countdown.minutes')}</p>
                      <p className="mt-1 text-xl font-semibold text-slate-50">{countdown.minutes}</p>
                    </div>
                    <div className="rounded-2xl border border-cyan-300/20 bg-cyan-400/10 px-3 py-3 text-center">
                      <p className="text-[10px] uppercase tracking-[0.18em] text-cyan-100/70">{t('countdown.seconds')}</p>
                      <p className="mt-1 text-xl font-semibold tabular-nums text-slate-50">{countdown.seconds}</p>
                    </div>
                  </>
                ) : (
            ...
            <div className="rounded-2xl border border-yellow-400/20 bg-yellow-500/10 p-4">
              ...
              <p className="mt-2 text-xl font-semibold text-slate-50">{formatToken(parsedUsdtPerVote)} USDT</p>
            </div>
            <div className="rounded-2xl border border-cyan-400/15 bg-cyan-400/10 p-4">
              ...
              <p className="mt-2 text-xl font-semibold text-slate-50">{formatToken(parsedRicoPerVote)} RICO</p>
            </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:col-span-2 xl:col-span-1">
                ...
                <p className="mt-2 text-xl font-semibold text-slate-50">{formatToken(globalVotes, 0)}</p>
                <div className="mt-3 flex items-center justify-between text-xs text-slate-400">
                  <span>{t('stats.usdtRaised')}</span>
                  <span className="font-medium text-slate-200">{formatToken(globalUsdt)} USDT</span>
                </div>
      ...
      <section className="theme-panel border border-yellow-400/20 bg-[linear-gradient(180deg,rgba(10,12,18,0.98),rgba(4,6,10,0.98))] p-5 md:p-6">
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
          <div>
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="theme-kicker mb-2">{t('setup.kicker')}</p>
                <h3 className="text-2xl font-semibold text-slate-50">{t('setup.title')}</h3>
            ...
            <div className="mt-5 grid gap-3 xl:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]">
              <div
            ...
            <div className="mt-6 rounded-[1.75rem] border border-white/10 bg-black/30 p-4 md:p-5">
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-2">
                <div className="rounded-[1.5rem] border border-yellow-400/20 bg-[linear-gradient(135deg,rgba(245,166,35,0.14),rgba(245,166,35,0.05))] px-4 py-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="rounded-2xl border border-yellow-300/20 bg-black/20 px-3 py-2 text-xs uppercase tracking-[0.16em] text-yellow-100/70">
                      {voteCount || 0}x
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-[1.5rem] border border-cyan-400/15 bg-[linear-gradient(135deg,rgba(0,212,255,0.14),rgba(0,212,255,0.04))] px-4 py-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="rounded-2xl border border-cyan-300/20 bg-black/20 px-3 py-2 text-xs uppercase tracking-[0.16em] text-cyan-100/70">
                      +{voteCount || 0}
                    </div>
                  </div>
                </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-2 text-xl font-semibold text-slate-50">{formatToken(parsedUsdtAllowance)} USDT</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="mt-5 rounded-[1.75rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))] p-4 md:p-5">
          ...
          <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-3xl border border-cyan-400/15 bg-[radial-gradient(circle_at_top,rgba(0,212,255,0.14),transparent_55%),linear-gradient(180deg,rgba(10,18,26,0.95),rgba(3,7,12,0.98))] p-5">
            ...
            <div className="rounded-3xl border border-yellow-400/20 bg-[linear-gradient(180deg,rgba(250,204,21,0.12),rgba(15,23,42,0.28))] p-5">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
                <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
                  ...
                  <p className="mt-2 text-2xl font-semibold text-slate-50">{formatToken(userVotes, 0)}</p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
              ...
              <dl className="mt-4 space-y-3 text-sm text-slate-300">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <dd className="font-medium text-slate-100">{shortAddress(owner as string | undefined)}</dd>
                </div>
                <div className="flex items-center justify-between gap-4">
